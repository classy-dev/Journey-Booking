  ...
  function search(...$args)
  {
      ...
      if(count($args) == 6 || count($args) == 7 || count($args) == 8) {
        ...
        $select2 = array(
          ...
          $this->pagination->initialize($config);
          $this->data['pagination'] = $this->pagination->create_links();
          $dataset = $this->Hotels_model->searchByLocation($city, $limit, $offset);
  ...
  function _search($country = null, $city = null, $citycode = null, $offset = null) {
    ...
    function book($hotelname)
    {
        ...
        if ($check && !empty($hotelname)) {
            ...
            $this->setMetaData($this->data['module']->title, $this->data['module']->metadesc, $this->data['module']->keywords);
            $this->theme->view('modules/hotels/booking', $this->data, $this);
