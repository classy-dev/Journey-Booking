...
class Flightsback extends MX_Controller {
		...
		public function viewdetails()
		{
			if(isset($_GET['booking_id']))
			{
				...
				$this->data['msg'] = $msg;
				$this->data['main_content'] = 'modules/bookings/view_details';
	            $this->data['page_title'] = 'Booking Details';
	            $this->load->view('Admin/template', $this->data);
			}
	        else {
	            redirect('admin/flightsbookings');
	        }
		...
		public function delete()
		{
			if(isset($_GET['booking_id']))
			{
				...
				$this->load->model('Flights_model');
				$this->Flights_model->delete_bookingData($booking_id);
		...
		{
			if(isset($_GET['booking_id']) && isset($_GET['status']) && ($_GET['status']=='cancel'))
			{

				$booking_id = $_GET['booking_id'];
			
                $this->load->model('Flights/Flights_model');
				$this->Flights_model->setstatus($_GET['booking_id'],'Cancelled');
				$booking_data = $this->Flights_model->get_booking_for_payment($booking_id);
			        $billing_data = $this->Flights_model->get_billing_for_payment($booking_id);
			        $pass_data = $this->Flights_model->get_passengerData($booking_id);
			        $segments_data = $this->Flights_model->get_segmentsData($booking_id);
			        ...
			        $this->Emails_model->SendCancelledEmail($booking_data);
                redirect('admin/flights/view/?booking_id='.$_GET['booking_id'].'&msg=Cancelled');
			}
		}
		...
		public function uploadTicket()
		{
            ...
            else
            {
                    ...
                    $this->load->model('Flights/Flights_model');
                    $this->Flights_model->setstatus($booking_id,'Ticket Issued');
			        ...
			        $this->load->model('Admin/Emails_model');
			        $this->Emails_model->SendticketEmail($booking_data,'uploads/tickets/'.$data['upload_data']['file_name']);

					redirect('admin/flights/view/?booking_id='.$booking_id.'&msg=issue');
            }
		}
		public function ResendTicket()
		{
			...
			$this->load->model('Flights/Flights_model');
			        ...
			        $this->load->model('Admin/Emails_model');
			        $this->Emails_model->SendticketEmail($booking_data,'uploads/tickets/'.$booking_id.'.pdf');
			redirect('admin/flights/view/?booking_id='.$_GET['booking_id'].'&msg=ticketresend');
		}
		public function SendVerificationEmail()
		{
			$this->load->model('Flights/Flights_model');
			$this->Flights_model->setstatus($_GET['booking_id'],'Verifying');
			$booking_id = $_GET['booking_id'];
			...
			$booking_data = $this->Flights_model->get_booking_for_payment($booking_id);
			        ...
			        $this->load->model('Admin/Emails_model');
			        $this->Emails_model->SendEmailVerificationEmail($booking_data);

			redirect('admin/flights/view/?booking_id='.$_GET['booking_id'].'&msg=Verifying');	
		}
		public function processing()
		{
			$this->load->model('Flights/Flights_model');
			$this->Flights_model->setstatus($_GET['booking_id'],'Processing');
			$booking_id = $_GET['booking_id'];
			...
			$booking_data = $this->Flights_model->get_booking_for_payment($booking_id);
			        ...
			        $this->load->model('Admin/Emails_model');
			        $this->Emails_model->SendEmailProcessing($booking_data);

			redirect('admin/flights/view/?booking_id='.$_GET['booking_id'].'&msg=Processing');	
		}
		public function flightsdiscounts()
		{
			$this->load->model('Flights/Flights_model');
			if(isset($_GET['dicsount_input']))
			{
				$this->Flights_model->setDiscounts($_GET['dicsount_input']);
			}	



			
			$discount = $this->Flights_model->getDiscounts();
			$this->data['discount'] = $discount;
			$form = '<form action="'.base_url().'admin/flightsdiscounts/" method="GET">
					<div class="row form-group">
			            <label class="col-md-2 control-label text-left" style="margin-top: 7px;">Discount (Place the Number)</label>
			            <div class="col-md-4">
			              <input name="dicsount_input" type="text" placeholder="Discount" class="form-control" value="'.$discount[0]->discount.'">
			            </div>
			          </div>
			          <div class="row form-group">
			...
			$this->data['page_title'] = 'Flights Discount';
            $this->data['main_content'] = 'temp_view';
            $this->data['header_title'] = 'Flights Discount';
            $this->load->view('Admin/template', $this->data);
		}
		public function flightsbookings()
		{
			$this->load->helper('xcrud');
	        $xcrud = xcrud_get_instance();
	        $xcrud->table('flightsbooking');

        	$xcrud->where('paidstatus', 'paid');
        	$xcrud->columns('booking_id,custfirstname,custlastname,location_from,location_to,flight_type,booking_datetime,total_price,bookingstatus');
	        $xcrud->order_by('ref_no', 'desc');
	        $xcrud->label('booking_id', 'Booking ID')->label('custfirstname', 'First Name')->label('custlastname', 'Last Name')->label('location_from', 'Departure')->label('location_to', 'Arrival')->label('flight_type', 'Type')->label('total_price', 'Total Price')->label('booking_datetime', 'Booking Date');
	        
	        $xcrud->search_columns('booking_id,custfirstname,custlastname,location_from,location_to,flight_type,booking_datetime,total_price');
	        $xcrud->button(base_url() . $this->data['adminsegment'] . '/flights/view/?booking_id={booking_id}', 'View Details', 'fa fa-search-plus', 'btn btn-primary', array('target' => '_blank'));
	        $xcrud->button(base_url() . $this->data['adminsegment'] . '/flights/delete/?booking_id={booking_id}', 'Delete Booking', 'fa fa-trash', 'btn btn-danger', array());
	        ...
	        $this->data['content'] = $xcrud->render();
	        $this->data['page_title'] = 'Flights Booking';
	        $this->data['main_content'] = 'temp_view';
	        $this->data['header_title'] = 'Flights Booking';
	        $this->load->view('Admin/template', $this->data);
		}
