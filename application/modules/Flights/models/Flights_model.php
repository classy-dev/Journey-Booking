...
class Flights_model extends CI_Model
{
    ...
    {
        $this->db->set('flightsbooking.bookingstatus', $status);
        $this->db->where('flightsbooking.booking_id', $id);
        $this->db->update('flightsbooking');
    }
    public function getemaildata()
    {
        $this->db->select('pt_app_settings.*');
        $result = $this->db->get('pt_app_settings')->result();
    ...
    public function getDiscounts()
    {
        ...
        $result = $this->db->get('discounts')->result();

        return $result;
    }
    public function setDiscounts($d)
    {
        $this->db->set('discounts.discount', $d);
        $this->db->where('discounts.id', 1);
    ...
    function generateRandomString($length = 6) 
    {
        ...
        $randomString.= $numbers[rand(0, $numberLength - 1)];
        return $randomString;
    }
    public function get_booking_for_payment($bookid)
    {
        $this->db->select('flightsbooking.*');
        $this->db->where('flightsbooking.booking_id', $bookid);
        $result = $this->db->get('flightsbooking')->result();

        return $result;
    }
    public function get_booking_for_status($bookid,$lastname)
    {
        $this->db->select('flightsbooking.*');
        $this->db->where('flightsbooking.booking_id', $bookid);
        $this->db->where('flightsbooking.custlastname', $lastname);
        $result = $this->db->get('flightsbooking')->result();

        return $result;
    }
    public function get_billing_for_payment($bookid)
    {
        $this->db->select('billinginfo.*');
        $this->db->where('billinginfo.booking_id', $bookid);
        $result = $this->db->get('billinginfo')->result();

        return $result;
    }
    public function get_passengerData($bookid)
    {
        $this->db->select('passengerdata.*');
        $this->db->where('passengerdata.booking_id', $bookid);
        $result = $this->db->get('passengerdata')->result();

        return $result;
    }
    public function get_segmentsData($bookid)
    {
        $this->db->select('segments.*');
        $this->db->where('segments.booking_id', $bookid);
        $result = $this->db->get('segments')->result();

        return $result;
    }
    public function update_booking_after_payment($bookid,$tran_id,$payment_method)
    {
        $data = array(
            "paidstatus" => 'paid',
        ...
        $this->db->update('flightsbooking', $data);
    }
    public function get_reject_booking_payment($bookid)
    {
        $this->db->select('flightsbooking.*');
        $this->db->where('flightsbooking.booking_id', $bookid);
        $result = $this->db->get('flightsbooking')->result();
    ...
    public function update_reject_booking_payment($bookid,$status)
    {
        $data = array(
    ...
    public function update_flights_billing()
    {
        
        
        $booking_id = $this->input->post('booking_id');
        $customerfirstname = $this->input->post('customerfirstname');
        $customermiddlename = $this->input->post('customermiddlename');
        ...
        $this->db->update('billinginfo', $booking_insert);
        
    }
    public function add_flights_billing()
    {
        $booking_id = $this->input->post('booking_id');
        $customerfirstname = $this->input->post('customerfirstname');
        $customermiddlename = $this->input->post('customermiddlename');
        ...
        $this->db->insert('billinginfo',$booking_insert);
        
    }
    public function add_flights_booking($data,$single_ticket_price)
    {
        $adults = $data['adults'];
        ...
        $booking_insert = array(
            ...
            "location_from_code" => $location_from_code,
            "location_to_code" => $location_to_code,
            "flight_type" => $flight_type
        );
        

        $this->db->insert('flightsbooking',$booking_insert);
        ...
        for($i=1;$i<=$adults;$i++)
        {
            $adultdata = array(
                "pass_type" => 'Adult',
                "pass_first_name" => $data["adultfirstname_".$i],
                "pass_middle_name" => $data["adultmiddlename_".$i],
                "pass_last_name" => $data["adultlastname_".$i],
                "pass_nationality" => $data["adultnationality_".$i],
                "pass_gender" => $data["genderadult_".$i],
                "pass_dob" => $data["adultdobdate_".$i],
                "pass_passport" => $data["adultidnumer_".$i],
                "booking_id" => $booking_id,
            );
            $this->db->insert('passengerdata',$adultdata);
        }
        for($i=1;$i<=$childs;$i++)
        {
            $adultdata = array(
                "pass_type" => 'Child',
                "pass_first_name" => $data["childfirstname_".$i],
                "pass_middle_name" => $data["childmiddlename_".$i],
                "pass_last_name" => $data["childlastname_".$i],
                "pass_nationality" => $data["childnationality_".$i],
                "pass_gender" => $data["genderchild_".$i],
                "pass_dob" => $data["childdobdate_".$i],
                "pass_passport" => $data["childidnumer_".$i],
                "booking_id" => $booking_id,
            );
            $this->db->insert('passengerdata',$adultdata);
        }
        for($i=1;$i<=$infants;$i++)
        {
            $adultdata = array(
                "pass_type" => 'Infant',
                "pass_first_name" => $data["infantsfirstname_".$i],
                "pass_middle_name" => $data["infantsmiddlename_".$i],
                "pass_last_name" => $data["infantslastname_".$i],
                "pass_nationality" => $data["infantsnationality_".$i],
                "pass_gender" => $data["genderinfants_".$i],
                "pass_dob" => $data["infantsdobdate_".$i],
                "pass_passport" => $data["infantsidnumer_".$i],
                "booking_id" => $booking_id,
            );
            $this->db->insert('passengerdata',$adultdata);
        }
        ...
        {
            $segment_data = array(
                "depart_code"   => $data["depart_code_".$i],
                "depart_name"   => $data["depart_name_".$i],
                "arrive_code"   => $data["arrive_code_".$i],
                "arrive_name"   => $data["arrive_name_".$i],
                "flight_leg_number"   => $data["flight_leg_number_".$i],
                "carrier_name"   => $data["carrier_name_".$i],
                "carrier_img"   => $data["carrier_img_".$i],
                "flight_number"   => $data["flight_number_".$i],
                "cabin_type"   => $data["cabin_type_".$i],
                "duration"   => $data["duration_".$i],
                "depart_date"   => $data["depart_date_".$i],
                "depart_time"   => $data["depart_time_".$i],
                "arrive_date"   => $data["arrive_date_".$i],
                "arrive_time"   => $data["arrive_time_".$i],
                "segment_type"   => $data["segment_type_".$i],
                "booking_id"   => $booking_id                
            );
            $this->db->insert('segments',$segment_data);
        }

        return $booking_id;

    }
    public function delete_bookingData($bookingid)
    {
        $this->db->where('booking_id', $bookingid);
        $this->db->delete('segments');
        $this->db->where('booking_id', $bookingid);
        $this->db->delete('billinginfo');
        $this->db->where('booking_id', $bookingid);
        $this->db->delete('passengerdata');
        $this->db->where('booking_id', $bookingid);
        $this->db->delete('flightsbooking');
    }
    ...
    function getApiRoutes($arrival_date, $departure_date, $type,$form,$to,$cabinclass_a,$checkTest)
    {
        ...
        {
            $this->db->group_start();
            $this->db->where('pt_flights_routes.date_arrival =', $arrival_date);
            $this->db->or_where('pt_flights_routes.date_arrival =', "0000-00-00");
            $this->db->group_end();
