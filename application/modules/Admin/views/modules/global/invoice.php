                        ...
                        <td align="left" style="padding-top: 20px;">
                            <table cellspacing="0" cellpadding="0" border="0" width="100%">
                                <?php if($invoice->module == "flights") {
                                    for($index = 0;$index<count($flight_data);$index++){
                                    ?>
                                    ...
                                    <div style="background:#ffffff;">
                                            ...
                                            <h5 style="margin: 10px 0; text-align: left; background: #eee; padding: 13px; color: #0068d7; text-transform: uppercase;"> <strong><?php if($index == 0) echo trans('0472'); else echo "Return";?></strong></h5>

                                                <table>
                                                    <tr>
                                                        <td><h5><strong><?php  echo trans('08');?></strong></h5></td>
                                                        <td><h5 class="strong"><?php echo $flight_data[$index]->aero_dep; ?></h5></td>

                                                        <td><h5 class="strong"><?php if(!empty(json_decode($flight_data[$index]->flight_no)[0])) { ?><?php echo "Type :" ;}?> <?php echo $item->type; ?> Class: <?php echo $flight_data[$index]->flight_type; ?></h5></td>
                                                    </tr>
                                                    <?php
                                                        $temp_array = array();
                                                       if($flight_data[$index]->date_departure == "0000-00-00")
                                                         $date = date("Y-m-d");
                                                      else
                                                         $date = $flight_data[$index]->date_departure;

                                                        $from_object = (object)["code"=>$flight_data[$index]->from_code,"locations"=>$flight_data[$index]->from_location,"date"=>$date];
                                                        array_push($temp_array,$from_object);
                                                        for($i=0;$i<count(json_decode($flight_data[$index]->transact));$i++) {
                                                            $trans_object = json_decode($flight_data[$index]->transact);
                                                            $date_trans = json_decode($flight_data[$index]->date_trans);
                                                            if(empty($date_trans[$i]))
                                                            {
                                                                $date = date("Y-m-d", strtotime(($i+1)." day", strtotime(date('Y-m-d'))));
                                                            }else{

                                                                $date = $date_trans[$i];
                                                            }
                                                            $from_object = (object)["code"=>json_decode($trans_object[$i])->code,"locations"=>json_decode($trans_object[$i])->label,"date"=>$date];
                                                            array_push($temp_array,$from_object);

                                                        }
                                                    if($flight_data[$index]->date_arrival == "0000-00-00")
                                                        $date = date("Y-m-d", strtotime(count($temp_array)." day", strtotime(date('Y-m-d'))));
                                                    else
                                                        $date = $flight_data[$index]->date_arrival;
                                                    $from_object = (object)["code"=>$flight_data[$index]->to_code,"locations"=>$flight_data[$index]->to_location,"date"=>$date];
                                                    array_push($temp_array,$from_object);

                                                    ?>
                                                        <?php for($j=0;$j<count($temp_array);$j++) {
                                                            ?>
                                                       <tr>
                                                        <td><p><?php echo $temp_array[$j]->date; ?></p></td>
                                                        <td><p><?php echo $temp_array[$j]->code;  ?></p></td>
                                                        <td><?php echo $temp_array[$j]->locations; ?>  </h5></td>
                                                    </tr>
                                                        <?php } ?>
                                                </table>
                                        <?php } ?>
                                        <h5 style="margin: 10px 0; text-align: left; background: #eee; padding: 13px; color: #0068d7; text-transform: uppercase;"> <strong>Passenger Data</strong></h5>
                                        <!-- Guest Info Table -->
                                ...
                                <tr class="well well-sm">
                                    <td width="75%" align="left" style="font-family: Tahoma; font-size: 16px; font-weight: 400; line-height: 14px; padding: 5px;">
                                        ...
                                        <span class="room-price"><?= $invoice->currSymbol.$subItem->price ?></span>
                                    </td>
