...
class Sabre extends MX_Controller {
    ...
    public $airLowFareSearchURI = 'v4.3.0/shop/flights?mode=live&limit=50&offset=0';
    ...
    public function index($searchForm = NULL)
    {
        ...
        $this->load->library('Model/Response/AirLowFareSearchRsp');
        $responseParser = new AirLowFareSearchRsp();
        // Load region and administration fee for the specified content.
        // $customFare = $this->get_custom_fee($searchForm->origin);
        $responseParser->customFare = new CustomFare(); //$customFare;
        
        $this->data['AirLowFareSearchRsp'] = $responseParser->parse($response->data);
        $this->data['pageTitle'] = "Search Results - From ".$searchForm->origin." to ".$searchForm->destination;
        $this->data['searchForm'] = $searchForm;
        $this->theme->view('modules/sabre/listing', $this->data, $this);
    }
    ...
    public function checkout()
    {
        ...
        $this->data['searchForm'] = $searchForm;
        $this->theme->view('modules/sabre/booking', $this->data, $this);
    }
