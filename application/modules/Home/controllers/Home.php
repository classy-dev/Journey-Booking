...
class Home extends MX_Controller {
    ...
    public function index() {
        ...
        if (!empty($spaSettings) && $spaSettings->spa_status == 'enable') {
            ...
            if (!empty($html)) {echo $html;return;}
        }
        ...
        if (isModuleActive('ean')) {
            $activeModules[] = "ean";

            $featuredSettings = $appSetting->getHomepageFeatureItems();

            if ($featuredSettings->ean_hotel_status == 'enable') {
...
public function b2b() {
    $this->setMetaData("B2B");
    $this->theme->view('home/b2b', $this->data, $this);
}
...
//        $this->data['modules'] = $this->ptmodules->moduleslist;
