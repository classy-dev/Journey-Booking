  ...
  $(function(){
    ...
    $(".trip-check label, .iCheck-helper").on('click', function() 
    {
      ...
      if (manualtriptype == 'oneway') {
        arrivalDatetimePicker.attr('disabled', 'disabled').css('background', '#d9d9d9');
        arrivalDatetimePicker.removeAttr('required');
        $('.onewaytype').html('<div class="detail"><div class="country choosen city">One Way</div><div class="country choosen">Set Return Type</div></div>')
        $('.onewaytype').show();
      } else {
        arrivalDatetimePicker.removeAttr('disabled').css('background', '#F9F9F9');
        arrivalDatetimePicker.attr('required', 'required');
    ...
    var now = new Date(nowTemp.getFullYear(), nowTemp.getMonth(), nowTemp.getDate(), 0, 0, 0, 0);
        ...
        }).on('changeDate', function(e)
        {
          ...
          var newDate = new Date(e.date);
          $('.deparuteTimeDiv').find('.flight-code span').html(newDate.getDate());
          $('.deparuteTimeDiv').find('.country').html(DayWeek[newDate.getDay()-1]);
          $('.deparuteTimeDiv').find('.city').html(Month[newDate.getMonth()]+", "+newDate.getFullYear());
        ...
        var arrivalTime = $('.arrivalTime').datepicker({
          ...
          $(this).datepicker('hide');
          var newDate = new Date(e.date);
          $('.arrivalTimeDiv').find('.flight-code span').html(newDate.getDate());
          $('.arrivalTimeDiv').find('.country').html(DayWeek[newDate.getDay()-1]);
          $('.arrivalTimeDiv').find('.city').html(Month[newDate.getMonth()]+", "+newDate.getFullYear());
        }).data('datepicker');

        $('.deparuteTimeDiv').click(function(){
            $('.departureTime').focus();
        });
        $('.arrivalTimeDiv').click(function(){
            $('.arrivalTime').focus();
        });
        // Default fill up date
        ...
        $('.totalManualPassengerDiv').click(function(){
            $('.guests .result').click();
        });
...
function k() {
        var t = $("#ticket-adult").val(),
            ...
            n = $("#qty-result");
            var n1 = $(".travellerclass");

            $("[name='madult']").val(parseInt(t, 10));
            $("[name='mchildren']").val(parseInt(i, 10));
            $("[name='minfant']").val(parseInt(r, 10));

        madult = parseInt($("[name='madult']").val()); // Should be 1
        mchildren = parseInt($("[name='mchildren']").val());
        minfant = parseInt($("[name='minfant']").val());
        totalManualPassenger = (madult + mchildren + minfant);
            
            $("[name='totalManualPassenger']").val(parseInt(t, 10) + parseInt(i, 10) + parseInt(r, 10));
            $('.totalManualPassengerDiv').find('.flight-code span').html(parseInt(t, 10) + parseInt(i, 10) + parseInt(r, 10));
            
        //n1.val(parseInt(t, 10) + parseInt(i, 10) + parseInt(r, 10));
    ...
    $(".qty-plus").add(".qty-minus").on("click", function(n) {
        ...
        var fromData = 'Enter City Or Airport';
        var toData = 'Enter City Or Airport';
        $.ajax({
            url: "<?php echo base_url(); ?>admin/ajaxcalls/flightSearchAjex/?query=<?php echo $query->origin; ?>",
 ...
 $.ajax({
            url: "<?php echo base_url(); ?>admin/ajaxcalls/flightSearchAjex/?query=<?php echo $query->destination; ?>",
            success: function (result, page) 
            {
                toData = result;
    ...
    <?php
    if(isset($_GET['adults']) && $_GET['adults']>0)
    {
        for($i=1;$i<=(int)$_GET['adults'];$i++)
        {
            ?>

            $('[name="adultnationality_<?php echo $i; ?>"]').select2(
                {
                  ...
                  ajax: 
                  {
                        url: "<?php echo base_url(); ?>admin/ajaxcalls/flightNationality",
                        dataType: 'json',
                        data: function (term, page) 
                        {
                          return {
    ...
    ?>
    <?php
    if(isset($_GET['child']) && $_GET['child']>0)
    {
        for($i=1;$i<=(int)$_GET['child'];$i++)
        {
            ?>

            $('[name="childnationality_<?php echo $i; ?>"]').select2(
                {
    ...
    ?>
    <?php
    if(isset($_GET['infants']) && $_GET['infants']>0)
    {
        for($i=1;$i<=(int)$_GET['infants'];$i++)
        {
            ?>
